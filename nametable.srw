Diagrams [@ds] = {{ support.resetNames (); }}[[${support.strMangle (ds)}]]
Diagram [k @a k2 graphmodel k3] = {{ scopeAdd ('diagramid', '') }}[[${a}${graphmodel}]]
Attribute [a] = [[${a}]]
NameAttribute [an k s] = [[]]
DiagramIDAttribute [an k s] = [[${support.newDiagramID (s, _scope)}\n${support.getDiagramID(_scope)}\t\t${s}\n]]
...
IDAttribute [id eq s] = [[${support.newCellID(s,_scope)}${support.getCellID(_scope)}\t\t${s}\n]]
